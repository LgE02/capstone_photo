...
사용 방법:
  # 원본 이미지 학습 (권장)
  python -m model_training.train_lora --config raw_only

  # after_03 재학습
  python -m model_training.train_lora --config after03
"""
...
# ── 설정 파일 동적 로드 ──────────────────────────────────────────────────────
# --config 인자를 먼저 파싱해 올바른 설정 모듈을 임포트한다.
_CONFIG_MAP = {
    "raw_only":       "train_config_raw_only",        # 원본 이미지만 사용 (권장)
}

_pre_parser = argparse.ArgumentParser(add_help=False)
_pre_parser.add_argument("--config", default="raw_only", choices=list(_CONFIG_MAP.keys()))
_pre_args, _ = _pre_parser.parse_known_args()

_config_module_name = _CONFIG_MAP[_pre_args.config]

def _load_config_module(module_name: str):
    """패키지 경로를 순서대로 시도해 설정 모듈을 로드한다."""
    candidates = [
        f"fairytale_lora.model_training.{module_name}",
        f"model_training.{module_name}",
        f"training.{module_name}",
        module_name,
    ]
    for candidate in candidates:
        try:
            ...
            continue
    raise ImportError(f"설정 모듈을 찾을 수 없습니다: {module_name} (시도한 경로: {candidates})")

_cfg = _load_config_module(_config_module_name)

DATASET           = _cfg.DATASET
LORA_NETWORK      = _cfg.LORA_NETWORK
MODEL             = _cfg.MODEL
OPTIMIZATION      = _cfg.OPTIMIZATION
PATHS             = _cfg.PATHS
STYLE_HINTS       = _cfg.STYLE_HINTS
TRAINING          = _cfg.TRAINING
VALIDATION_PROMPTS = _cfg.VALIDATION_PROMPTS
...
class FairytaleDataset(Dataset):
    ...
    def _enhance_caption(self, img_path: Path, caption: str) -> str:
        """이미지별 캡션에 스타일 힌트를 추가한다.

        DATASET["captions_are_complete"] = True 이면 캡션이 이미 완성된 상태이므로
        파일명 기반 힌트 추가를 건너뛴다 (after03 재학습 데이터셋 등).
        """
        # 캡션이 이미 완성된 데이터셋 (예: retrain_after03) — 그대로 반환
        if DATASET.get("captions_are_complete"):
            return caption
        # style_core 디렉터리도 별도 처리 없이 그대로 사용
...
def parse_args():
    ...
    parser.add_argument(
        "--config",
        default="raw_only",
        choices=list(_CONFIG_MAP.keys()),
        help="학습 설정 (raw_only: 원본 이미지, after03: after_03 스타일)",
    )
